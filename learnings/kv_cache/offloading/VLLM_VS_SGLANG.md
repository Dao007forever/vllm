...
vLLM requests, allocated GPU block IDs, `SchedulerOutput`, and connector
completion sets. This makes it efficient to wire into vLLM, but it also means
Mooncake policy and vLLM scheduler details are intertwined.
...
SGLang's `PoolTransfer` model is broader. It can tie KV page hits to auxiliary
state pools and decide whether those pools must exist for every page or only
for trailing pages. This is a better fit for hybrid models that need more than
K/V tensors to be restored correctly.
